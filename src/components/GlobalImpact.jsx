...
const GlobalImpact = () => {
  // Refs for the tech hubs and impact zones
  const northAmericaRef = useRef(null);
  const europeRef = useRef(null);
  const africaRef = useRef(null);
  const asiaRef = useRef(null);
  const southAmericaRef = useRef(null);

  // Define the connections between tech hubs and impact zones
  const connections = [
    {
      from: northAmericaRef,
      to: africaRef,
      delay: 1.2,
      startAnchor: "right",
      endAnchor: "left",
      color: "url(#techToImpactGradient)"
    },
    {
      from: europeRef,
      to: africaRef,
        ...
        >
          <div className="bg-white/5 backdrop-blur-md border border-electric-cyan/20 rounded-3xl p-8 md:p-12 relative overflow-hidden">
            ...
            <div className="relative w-full h-[500px] md:h-[700px] bg-midnight-blue/50 rounded-2xl overflow-hidden">
              ...
              {/* Xwrapper for the arrows */}
              <Xwrapper>
                {/* Define the gradient for the connection lines */}
                <svg style={{ position: 'absolute', width: 0, height: 0 }}>
                  <defs>
                    <linearGradient id="techToImpactGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                      <stop offset="0%" stopColor="#00D9FF" /> {/* Blue at tech hub */}
                      <stop offset="100%" stopColor="#34D399" /> {/* Green at impact zone */}
                    </linearGradient>
                  </defs>
                </svg>

                {/* Draw all connection lines with arrows */}
                {connections.map((connection, index) => (
                  <Xarrow
                    ...
                    showHead={true}
                    showTail={false}
                    headShape={{ svg: "M 0 0 L 10 5 L 0 10 z" }}
                    headSize={10}
                    animateDrawing={{
                      duration: 2000,
                ...
                <div className="absolute top-6 left-1/2 transform -translate-x-1/2 text-center z-30 w-full px-4">
                  <motion.div
                    initial={{ opacity: 0, y: -20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ duration: 0.8, delay: 0.5 }}
                    className="bg-midnight-blue/90 backdrop-blur-md border-2 border-electric-cyan/50 rounded-2xl px-8 py-4 shadow-2xl max-w-md mx-auto"
                  >
